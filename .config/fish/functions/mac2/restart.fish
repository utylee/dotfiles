function restart
    function __pick_domain
        set -l uid (id -u)
        # gui domain 존재하면 gui 우선
    ...
    function __restart_help
        ...
        echo ""
        echo "notes:"
        echo "  - Do NOT run with sudo (must run as your login user)"
        echo "  - Uses gui/<uid> if available; falls back to user/<uid>"
    end
    ...
    set -l domain (__pick_domain)
    set -l service $domain/$label
    set -l plist "$HOME/Library/LaunchAgents/$label.plist"

    if test "$mode" = "soft"
        echo "↻ soft restart $label ($domain)"
        launchctl kickstart -k $service
        return $status
    end

    if not test -f $plist
        echo "restart: plist not found: $plist"
        return 1
    end

    echo "↻ reload+restart $label ($domain)"
